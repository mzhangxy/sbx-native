...
// ======================== 终极内存限制 ========================
// 强制压榨底层 Go 插件的内存空间
process.env.GOGC = '20';          // 默认100，改为20让Go极其频繁地回收内存
process.env.GOMEMLIMIT = '20MiB'; // 强制Go运行时软内存上限限制在 20MB
process.env.DISABLE_ARGO = 'true';// 强制禁用最吃内存的 Argo 隧道
// ==============================================================
...
const ARGO_PORT      = Number(process.env.ARGO_PORT) || 59001;   
const S5_PORT        = process.env.S5_PORT        || '';         
const TUIC_PORT      = process.env.TUIC_PORT      || '';         
const HY2_PORT       = process.env.HY2_PORT       || '6028';     // 启用 HY2 端口
const ANYTLS_PORT    = process.env.ANYTLS_PORT    || '';         
const REALITY_PORT   = process.env.REALITY_PORT   || '';         // 留空，不再启用 Reality
...
function generateSingBoxConfig(certPath, keyPath) {
  ...
  // 极简返回，彻底舍弃消耗内存的 Wireguard (WARP) 和沉重的路由规则
  return {
...
async function startServer() {
  ...
  setTimeout(() => {
    ...
    if (global.gc) {
      global.gc();
      console.log('Initial garbage collection triggered.');
    }
...
// ======================== 持续内存回收 ========================
// 每 30 秒强制回收一次 Node.js 垃圾内存，确保不超限
setInterval(() => {
  if (global.gc) {
    global.gc();
  }
